...
class MetricLogger(object):
    def __init__(self, delimiter="\t",my_wandb= None):
    ...
    def log_every(self, iterable,iterable_t, print_freq, header=None, logger=None,train_state=False,if_wandb=False,task="c2f"):
        if logger is None:
            print_func = print
        else:
        ...
        MB = 1024.0 * 1024.0
    # city2foggy 
        if train_state and task=="c2f"  :
          for obj1,obj2 in zip(iterable,iterable_t):
              data_time.update(time.time() - end)
              yield obj1,obj2 
              # import ipdb; ipdb.set_trace()
              iter_time.update(time.time() - end)
              if i % print_freq == 0 or i == len(iterable_t) - 1:
                  eta_seconds = iter_time.global_avg * (len(iterable_t) - i)
                  eta_string = str(datetime.timedelta(seconds=int(eta_seconds)))
                  if torch.cuda.is_available():
                      print_func(log_msg.format(
                          i, len(iterable_t), eta=eta_string,
                          meters=str(self),
                          time=str(iter_time), data=str(data_time),
                          memory=torch.cuda.max_memory_allocated() / MB))
                      if train_state and not if_wandb:
                        self.my_wandb.log({
                          "loss": self.meters['loss'].global_avg,
                          "loss_rt": self.meters['loss'].avg
                          })
                  else:
                      print_func(log_msg.format(
                          i, len(iterable_t), eta=eta_string,
    ...
    #sim10k2city city2bdd100k
        elif train_state and (task=="s2c" or task=="c2b"):
          iterable_t_cycle = iter(iterable_t)
          for obj1 in iterable:
              data_time.update(time.time() - end)
              try:
                  obj2 = next(iterable_t_cycle)
