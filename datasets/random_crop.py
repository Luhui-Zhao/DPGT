...
def random_crop(image1, image2, boxes, labels, difficulties=None):
    '''
        image: A PIL image
    ...
    '''
    if type(image1) == PIL.Image.Image:
        image1 = F.to_tensor(image1)
    if type(image2) == PIL.Image.Image:
    ...
    while True:
        ...
        if mode is None:
            return F.to_pil_image(image1), F.to_pil_image(image2),boxes, labels, difficulties
        ...
        for _ in range(50):
            ...
            #Crop
            new_image1 = image1[:, int(top):int(bottom), int(left):int(right)] #(3, new_h, new_w)
            new_image2 = image2[:, int(top):int(bottom), int(left):int(right)]  # (3, new_h, new_w)
            ...
            return F.to_pil_image(new_image1),F.to_pil_image(new_image2), new_boxes, new_labels, new_difficulties