...
#target
Prompt_vectors_t = [[] for _ in range(1)]
area_ratios_t = [[] for _ in range(1)]

#target false
Prompt_vectors_tf = [[] for _ in range(1)]
area_ratios_tf = [[] for _ in range(1)]
debug_id=0
num_class=1
num_scale=50
# Training
def train_one_epoch(model: torch.nn.Module,model_t: torch.nn.Module, criterion: torch.nn.Module,postprocessors,data_loader_s: Iterable,data_loader_t: Iterable, optimizer: torch.optim.Optimizer,
    ...
    for data_s_sf,data_t_tf in metric_logger.log_every(data_loader_s,data_loader_t, print_freq, header, logger=logger,train_state=train_state,if_wandb=args.debug,task="s2c"):
