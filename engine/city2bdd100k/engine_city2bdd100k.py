...
#target
Prompt_vectors_t = [[] for _ in range(7)]
area_ratios_t = [[] for _ in range(7)]

#target false
Prompt_vectors_tf = [[] for _ in range(7)]
area_ratios_tf = [[] for _ in range(7)]
debug_id=0
num_class=7
num_scale=5
# Training
def train_one_epoch(model: torch.nn.Module,model_t: torch.nn.Module, criterion: torch.nn.Module,postprocessors,data_loader_s: Iterable,data_loader_t: Iterable, optimizer: torch.optim.Optimizer,
    ...
    for data_s_sf,data_t_tf in metric_logger.log_every(data_loader_s,data_loader_t, print_freq, header, logger=logger,train_state=train_state,if_wandb=args.debug,task="c2b"):
...
def tab_display(Prompt_vectors_tf,Prompt_vectors_t,total_tf,total_t):
  category_instancesonly = [ 'person', 'car', 'bus', 'rider', 'truck', 'motor', 'bike']
