...
class DABDeformableDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels,
        ...
        #domain adaption
        self.class_feature_s=torch.zeros(num_classes, 256)
        self.class_feature_m=torch.zeros(num_classes, 256)
        self.class_feature_l=torch.zeros(num_classes, 256)
    ...
    def forward(self, samples: NestedTensor):
        ...
        features, pos = self.backbone(samples)#backbone输入
        ...
        out = {'pred_logits': outputs_class[-1], 'pred_boxes': outputs_coord[-1],'hs_query':hs[5].detach()}
        if self.aux_loss:
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        results = [{'scores': s, 'labels': l, 'boxes': b,'id_query': q} for s, l, b,q in zip(scores, labels, boxes,topk_boxes)]

        return results

class PostProcess_eval(nn.Module):
    """ This module converts the model's output into the format expected by the coco api"""
...
def build_dab_deformable_detr(args):
    if args.da_task=="city2foggy":
      ...
      num_classes=8
    # num_classes=9
    print("The classification layer is divided into several categories:"+str(num_classes))
    ...
    postprocessors_eval = {'bbox': PostProcess_eval()}
    if args.masks:
