...
class AdjustContrast:
    ...
    def __call__(self, image1, target1,image2, target2):
        ...
        image1 = F.adjust_contrast(image1, _contrast_factor)
        image2 = F.adjust_contrast(image2, _contrast_factor)
...
class AdjustBrightness:
    ...
    def __call__(self, image1, target1,image2, target2):
        ...
        image1 = F.adjust_brightness(image1, _brightness_factor)
        image2 = F.adjust_brightness(image2, _brightness_factor)
        return image1, target1,image2, target2

def lighting_noise(image1,image2):
    '''
    ...
    new_image1 = F.to_tensor(new_image1)
    new_image2 = F.to_tensor(new_image2)
    new_image1 = new_image1[swap, :, :]
    new_image2 = new_image2[swap, :, :]
    new_image1 = F.to_pil_image(new_image1)
    new_image2 = F.to_pil_image(new_image2)
...
class LightingNoise:
    ...
    def __call__(self, image1, target1,image2, target2):
        return lighting_noise(image1,image2)[0], target1,lighting_noise(image1,image2)[1],target2
...
class RandomCrop:
    ...
    def __call__(self, image1, target1,image2, target2):
        ...
        #     import ipdb; ipdb.set_trace()
        try:
            boxes_xyxy = target1['boxes']
            labels = target1['labels']
            image1, image2, new_boxes, new_labels, _ = random_crop(image1, image2,boxes_xyxy, labels)
            target1['boxes'] = new_boxes
            target1['labels'] = new_labels
            target2['boxes'] = new_boxes
            target2['labels'] = new_labels
...
class RandomSelectMulti(object):
    ...
    def __call__(self, image1, target1,image2, target2):
        if self.p == -1:
            return random.choice(self.transformslist)(image1, target1,image2, target2)
